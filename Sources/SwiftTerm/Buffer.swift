...
public final class Buffer {
    ...
    /// Absolute index of the first stored buffer line.
    ///
    /// In normal terminal output this increases when output pushes a line out
    /// of a full scrollback buffer (`Terminal.scroll`) and resets to zero on
    /// hard reset (RIS) and buffer (re)construction. Host-side scrollback
    /// hydration may prepend older captured rows, which moves this value
    /// backward. Embedders can use it to anchor a scrolled-up viewport by
    /// absolute buffer line rather than pixel offset.
