...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    /// `traitCollectionDidChange(_:)`. The first apply only happens when this
    /// property is set (it is not invoked from init), which preserves the
    /// init-time `nativeBackgroundColor = .clear` transparency setup. Named
    /// with the `terminal` prefix to match the macOS API.
    ...
    // Keep the fork's public UIScrollViewDelegate hooks source-compatible
    // while routing them through the upstream content-offset state machine.
