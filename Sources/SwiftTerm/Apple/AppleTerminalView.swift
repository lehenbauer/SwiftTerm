...
extension TerminalView {
    ...
    func updateDisplay (notifyAccessibility: Bool)
    {
        ...
        if metalView != nil {
            ...
            if buffer.lines.count == 0 {
                ...
                dirtyRange = absoluteDependencyRange
            } else {
    ...
    func updateCursorPosition()
    {
        ...
        #endif
        let charUnderCursor = buffer.lines [vy][cursorColumn]
    ...
    private func updateUserScrollingState(for row: Int, in displayBuffer: Buffer) {
        let maxScrollback = max(0, displayBuffer.lines.count - displayBuffer.rows)
        let isUserScrolling = !terminal.isDisplayBufferAlternate && row < maxScrollback
